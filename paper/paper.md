...
algorithm [@abrahams_2024] for simultaneously tiling and augmenting large, scientific images in preparation 
for use with deep-learning algorithms. Due to the onboard memory limitations of GPUs, large images need to be sliced into 
...
this issue is avoided by sliding a tiling window across multiple overlaps at a single pixel location, but this solution
...
unique, physically realistic permutation. This strategy not only removes redundancies, but extends the training 
...
whatever size is needed---all from a single line of code. Although Flip-n-Slide is the first tiling strategy 
...
accomplishments of deep neural networks in a variety of vision classification tasks [@ronneberger_2015a; @zhao_2017; 
...
address overfitting through data mixing (e.g., [@zhang_2017; @yun_2019; @hong_2021; @baek_2021]) or proxy-free 
augmentations (e.g., [@cubuk_2019; @reed_2021; @li_2023])---strategic approaches that expand the training 
...
for GPU memory [@pinckaers_2018; @huang_2019]. 
...
the accepted convention (with a typical overlap of 50%) and has been incorporated at training time as well 
[@unel_2019; @zeng_2019; @reina_2020; @akyon_2022]. However simply overlapping tiles has two drawbacks: (1) overlapping tiles 
introduce redundancies at training time as many pixel windows are repeated more than once, leading to overfitting; and (2)
tiling can slice through objects which removes context from the slices at training and test time.

[@charette_2021] addressed issues with object slicing by combining objects into a super-object in post-processing when 
a first learning iteration indicates that adjacent tiles contain the same object. [@nguyen_2023] extended this approach 
with a dynamic tiling routine that employs an iterative process to detect a dynamic size to assign to 
object-of-interest (OoI) patches after pre-learning on fixed, non-overlapping tiles; however this process creates an initial 
training stage for every test image, requiring large overhead. Although both of these approaches show promise in preserving 
...
to disk as either arrays or tensors. Our goal in writing this codebase was to enable a user to modularly include this tiling method 
...
We include a verbosity flag for all of these classes, which is especially useful when using `flipnslide` to download, preprocess, 
then tile and augment an image. We have optimized this approach to be used with large imagery, and large images can take extended 
...
who are new to working with GPU-enabled datasets to be able to include their data directly into `flipnslide` and within a few lines 
